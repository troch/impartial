...
function Element (token, parent) {
    this.name = token.type === 'element' ? token.val[0] : token.type === 'script' ? 'script' : null;
    this.attrs = [];
...
function buildElementTree (tokens) {
    ...
    for (i in tokens) {
        ...
        if (token.type === 'doctype') {
            htmlDocument = new Document(token.val);
        } else if (token.type === 'element' || token.type === 'text' || token.type === 'script') {
